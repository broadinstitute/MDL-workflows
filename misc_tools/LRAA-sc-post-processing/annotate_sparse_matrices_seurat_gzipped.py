...
'''
WDL-Compatible Seurat Format Annotation Script with Gzipped Output

Usage:
python3 annotate_sparse_matrices_seurat_gzipped.py \
...
def annotate_sparse_matrix(matrix_dir, gene_mapping, output_dir, matrix_type="gene"):
    ...
    # Write annotated features in Seurat format (2 columns, no header, tab-separated, GZIPPED)
    output_features_file = os.path.join(output_dir, 'features.tsv.gz')
    
    try:
        # Write gzipped for Seurat compatibility
        with gzip.open(output_features_file, 'wt') as f:
            output_df.to_csv(f, sep='\t', header=False, index=False, quoting=0)
        ...
        # Verify the gzipped file can be read
        print(f"Verifying gzipped output:")
        with gzip.open(output_features_file, 'rt') as f:
...
def main():
    parser = argparse.ArgumentParser(
        description="Annotate sparse matrices with gene symbols in Seurat-compatible format with gzipped output",
    ...
    print("=" * 80)
    print("SEURAT-COMPATIBLE ANNOTATION SCRIPT (GZIPPED OUTPUT)")
    print("=" * 80)
    ...
    if success_gene and success_isoform:
        ...
        print("Output format: Seurat-compatible (2-column features.tsv.gz)")
        print("Format: gene_id <TAB> gene_symbol^gene_id")
        print("\nExpected output structure:")
        print("sample^gene-sparseM.annotated/")
        print("├── barcodes.tsv.gz")
        print("├── features.tsv.gz    # ← GZIPPED for Seurat")
        print("└── matrix.mtx.gz")
        print("\nSeurat usage:")
